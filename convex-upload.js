// Upload resources with HTML content to Convex
// Run: node convex-upload.js
    ...
    for (const course of courses) {
      ...
      for (const file of files) {
        ...
        resources.push({
          ...
          fileType: 'html',
          fileSize,
          content, // Store the full HTML content
          programId,
          accessType: 'enrolled',
          sortOrder,
        });
...
async function main() {
  ...
  console.log(`📦 Found ${resources.length} resources with HTML content\n`);
  
  // Check total content size
  const totalContentSize = resources.reduce((sum, r) => sum + (r.content?.length || 0), 0);
  console.log(`📄 Total content size: ${(totalContentSize / 1024 / 1024).toFixed(2)} MB\n`);
  
  const convexUrl = 'https://terrific-dove-836.convex.cloud';
  const client = new ConvexClient(convexUrl);
  
  // Upload in batches of 20 (smaller due to content)
  const batchSize = 20;
  let totalUploaded = 0;
  ...
  for (let i = 0; i < resources.length; i += batchSize) {
    ...
    try {
      const result = await client.mutation('resources:bulkCreateResourcesInternal', {
        resources: batch.map(r => ({
          title: r.title,
      ...
      console.log(`   ✅ Batch ${batchNum}/${totalBatches}: ${result.count} resources`);
    } catch (error) {
  ...
  if (totalUploaded > 0) {
    console.log(`\n✅ Resources with HTML content uploaded to Convex!`);
    console.log(`   Content is stored in the database, not on disk.`);
  }
