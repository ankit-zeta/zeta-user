...
/**
 * Public entry point that rebuilds the Digital Business Execution (₹8,000)
 * program: generates the 24 Business Execution Toolkit PDFs, stores them in
 * Convex storage (storage.store is only available in actions), then runs
 * businessSetup:upsertBusinessCurriculumDb to rebuild the curriculum.
 */
export const upsertBusinessCurriculum: RegisteredAction<
  "public",
...
> = action({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      return await ctx.runMutation(internal.businessSetup.upsertBusinessCurriculumDb, {
        token: args.token,
        resetUserProgress: args.resetUserProgress,
        resourceStorageIds,
      });
    } catch (err: any) {
      throw new Error(`upsertBusinessCurriculum failed: ${err?.message || String(err)}`);
    }
  },
});