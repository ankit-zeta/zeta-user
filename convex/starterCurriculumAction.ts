...
import { STARTER_RESOURCES } from "./starterCurriculum";
...
/**
 * Public entry point that rebuilds the Starter Digital Skills (₹2,000)
 * program: generates the Starter Bundle PDFs, stores them in Convex
 * storage (storage.store is only available in actions), then runs
 * starterSetup:upsertStarterCurriculumDb to rebuild the curriculum.
 */
export const upsertStarterCurriculum: RegisteredAction<
  "public",
  ...
  handler: async (ctx, args) => {
    try {
      ...
      for (const resDef of STARTER_RESOURCES) {
        ...
        resourceStorageIds[resDef.fileName] = storageId;
      }

      return await ctx.runMutation(internal.starterSetup.upsertStarterCurriculumDb, {
        token: args.token,
        resetUserProgress: args.resetUserProgress,
        resourceStorageIds,
      });
    } catch (err: any) {
      throw new Error(`upsertStarterCurriculum failed: ${err?.message || String(err)}`);
    }
  },
});