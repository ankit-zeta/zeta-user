...
/**
 * Public entry point that rebuilds the Digital Business Pro (₹14,000)
 * program: generates the 40 Pro Resource Library PDFs, stores them in
 * Convex storage (storage.store is only available in actions), then runs
 * proSetup:upsertProCurriculumDb to rebuild the curriculum.
 */
export const upsertProCurriculum: RegisteredAction<
  "public",
...
> = action({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      return await ctx.runMutation(internal.proSetup.upsertProCurriculumDb, {
        token: args.token,
        resetUserProgress: args.resetUserProgress,
        resourceStorageIds,
      });
    } catch (err: any) {
      throw new Error(`upsertProCurriculum failed: ${err?.message || String(err)}`);
    }
  },
});