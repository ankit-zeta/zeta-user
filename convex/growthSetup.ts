...
import { GROWTH_MODULES, GROWTH_PROGRAM_PATCH } from "./growthCurriculum";
import { GROWTH_RESOURCES } from "./growthResources";

/**
 * Rebuilds the Growth Professional (₹4,000) program with the complete
 * text-based masterclass (12 modules / 12 lessons) and the 28-file
 * Growth Bundle. Idempotent: it replaces the program's existing modules,
 * lessons, resources, and demo progress. Storage IDs for the bundle PDFs
 * are prepared by the public action growthCurriculumAction:upsertGrowthCurriculum.
 */
export const upsertGrowthCurriculumDb = internalMutation({
  args: {
    token: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    const program = await ctx.db
      .query("programs")
      .withIndex("by_slug", (q) => q.eq("slug", "growth-professional"))
      .first();
    if (!program) {
      throw new Error("Growth program not found. Run seedDatabase first.");
    }
    ...
    const lessonIds = [];
    let totalMinutes = 0;
    for (const moduleDef of GROWTH_MODULES) {
    ...
    return {
      message: "Growth curriculum rebuilt successfully.",
      modules: GROWTH_MODULES.length,
