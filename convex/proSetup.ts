...
import { PRO_MODULES, PRO_PROGRAM_PATCH } from "./proCurriculum";
import { PRO_RESOURCES } from "./proResources";

/**
 * Rebuilds the Digital Business Pro (₹14,000) program with the complete
 * text-based masterclass (18 modules / 18 lessons) and the 40-file Pro
 * Resource Library. Idempotent: it replaces the program's existing modules,
 * lessons, resources, and demo progress. Storage IDs for the bundle PDFs
 * are prepared by the public action proCurriculumAction:upsertProCurriculum.
 */
export const upsertProCurriculumDb = internalMutation({
  args: {
    token: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    const program = await ctx.db
      .query("programs")
      .withIndex("by_slug", (q) => q.eq("slug", "premium-master-program"))
      .first();
    if (!program) {
      throw new Error("Pro program not found. Run seedDatabase first.");
    }
    ...
    const lessonIds = [];
    let totalMinutes = 0;
    for (const moduleDef of PRO_MODULES) {
    ...
    const resourceIds = [];
    for (const resDef of PRO_RESOURCES) {
    ...
    return {
      message: "Pro curriculum rebuilt successfully.",
      modules: PRO_MODULES.length,
